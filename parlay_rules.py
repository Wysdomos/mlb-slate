...
# Yard Sale is an explicitly HR-only parlay product. Other sections stay off
# HR legs until calibration promotes a replacement for the inverted HR ranker.
HR_MARKET_EXCEPTIONS = {"yard_sale_same_game", "yard_sale_cross_game"}
...
def validate_parlay(
    ...
    *,
    ...
    for leg in legs:
        ...
        if market == "HR" and correlation_type not in HR_MARKET_EXCEPTIONS:
            return False, "HR legs are only allowed in Yard Sale"

    ranked = sorted(
        legs,
        key=lambda leg: float(leg.get("confidence_rank", 999) or 999),
    )
    if ranked and leg_market(ranked[0]) == "HR" and correlation_type not in HR_MARKET_EXCEPTIONS:
