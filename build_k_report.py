...
Phase 2 (pybaseball — unlocks when k_savant_data.json is present):
  7-12. SwStr%, Chase%, Arsenal Whiff%, Lineup K%, H/A Split, Recent Form x5
...
for sp in SP_PROJ:
    ...
    c1 = grade(inn,     5.5, 5.0)
    c2 = grade(k_proj,  5.0, 4.5)
    c3 = grade_qs(qs)
    c4 = grade(opp_k,   9.0, 8.0)
    c5 = grade_inv(proj_bb, 2.0, 3.0)
    c6 = grade(bf,     22.0, 20.0)
    ...
    if PHASE2_ACTIVE and savant:
        c7  = grade(sf(savant.get('swstr_pct')),  11.0, 10.0)
        c8  = grade(sf(savant.get('chase_pct')),  29.0, 27.0)
        c9  = grade(sf(savant.get('arsenal_whiff')), 30.0, 28.0)
        c10 = grade(sf(savant.get('opp_lineup_k_pct')), 24.0, 22.0)
        c11 = grade(sf(savant.get('ha_split')),   0.5, 0.3)
        c12 = grade(sf(savant.get('recent_form')), 5.5, 4.5)
        score += sum(1 for c in [c7, c8, c9, c10, c11, c12] if c == 'green')
        total_pts = 12
        p7_val  = f'{sf(savant.get("swstr_pct")):.1f}%'
        p8_val  = f'{sf(savant.get("chase_pct")):.1f}%'
        p9_val  = f'{sf(savant.get("arsenal_whiff")):.1f}%'
        p10_val = f'{sf(savant.get("opp_lineup_k_pct")):.1f}%'
        p11_val = f'{sf(savant.get("ha_split")):+.1f}K'
        p12_val = f'{sf(savant.get("recent_form")):.1f}'
    else:
    ...
    pitchers.append({
        ...
        'c11':(c11, 'H/A',     p11_val), 'c12':(c12, 'Form x5',  p12_val),
...
# ---- HTML helpers ----------------------------------------------------------
TIER_COLOR = {'DIAMOND':'#3b82f6','ELITE':'#22c55e','STRONG':'#eab308','BORDERLINE':'#6b7280','FADE':'#374151'}
...
def pitcher_card(p, idx):
    ...
    flag_html = ''.join(f'<div class="flag flag-{f[2]}">{f[0]} {f[1]}</div>' for f in p['flags'])
    pk_color = p['c2'][0]
    ...
    row3_cells = ''.join([
        ...
        crit_box(p['c12'][0] if PHASE2_ACTIVE else None, 'Form x5',  p['c12'][2] if PHASE2_ACTIVE else None),
    ...
    return (f'<article class="pcard tier-{tier_cls}">'
            ...
            f'</div></div>'
            f'{row1}{row2}{row3}'
            f'{"<div class=flags>" + flag_html + "</div>" if flag_html else ""}'
            f'</article>')
...
def qual_row(p):
    ...
    return (f'<div class="qrow tier-{p["tier"].lower()}-border">'
            ...
            f'{"<span>" + flags_txt + "</span>" if flags_txt else ""}'
            f'</div></div>')
...
body{{background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"SF Pro Text",system-ui,sans-serif;font-size:14px;line-height:1.5;padding-bottom:60px;-webkit-font-smoothing:antialiased;}}
.top-nav{{background:rgba(10,22,18,0.96);backdrop-filter:blur(12px);border-bottom:1px solid var(--border);padding:12px 16px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;}}
...
.how-toggle{{width:100%;background:var(--card);border:1px solid var(--border);border-radius:var(--rs);padding:12px 14px;display:flex;justify-content:space-between;align-items:center;color:var(--text);font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;}}
...
.how-body{{display:none;background:var(--card);border:1px solid var(--border);border-top:none;border-bottom-left-radius:var(--rs);border-bottom-right-radius:var(--rs);padding:16px 14px;flex-direction:column;gap:16px;}}
...
.how-row{{display:flex;gap:10px;align-items:flex-start;padding:8px 10px;background:var(--glass);border-radius:6px;border:1px solid var(--border);}}
...
.rule-badge{{font-size:11px;font-weight:700;padding:4px 10px;border-radius:999px;border:1px solid;white-space:nowrap;flex-shrink:0;}}
.rule-desc{{font-size:12px;color:var(--soft);}}
.snav{{display:flex;gap:8px;padding:12px 16px;overflow-x:auto;border-bottom:1px solid var(--border);scrollbar-width:none;}}
.snav::-webkit-scrollbar{{display:none;}}
.snav a{{white-space:nowrap;padding:7px 14px;border-radius:999px;border:1px solid var(--border);background:var(--card);color:var(--soft);font-size:12px;font-weight:600;text-decoration:none;}}
.snav a.on{{background:var(--accent-bg);border-color:var(--accent-bd);color:var(--accent);}}
.sec{{padding:0 16px;margin-top:20px;}}
.sec-hd{{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--dim);margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;}}
.sec-hd span{{color:var(--soft);font-size:11px;font-weight:400;text-transform:none;letter-spacing:0;}}
.mnote{{background:var(--accent-bg);border:1px solid var(--accent-bd);border-radius:var(--rs);padding:12px 14px;margin-bottom:16px;font-size:12px;color:var(--soft);line-height:1.6;}}
.mnote strong{{color:var(--accent);}}
.pcard{{background:var(--card);border:1px solid var(--border);border-radius:var(--r);margin-bottom:10px;overflow:hidden;}}
...
.ou-badge{{font-size:14px;font-weight:900;padding:6px 14px;border-radius:8px;text-align:center;white-space:nowrap;}}
...
.cl{{font-size:8px;color:#ffffff;display:block;margin-top:2px;text-transform:uppercase;letter-spacing:.02em;font-weight:600;}}
...
.qrow{{display:flex;justify-content:space-between;align-items:center;background:var(--card);border:1px solid var(--border);border-radius:var(--rs);padding:10px 12px;margin-bottom:8px;gap:10px;}}
...
.group-label{{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--dim);margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid var(--border);}}
.parlay-list{{display:flex;flex-direction:column;gap:10px;}}
.pcard-parlay{{background:var(--card);border:1px solid var(--border);border-radius:var(--r);padding:14px;}}
.ph{{display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;}}
.pl{{font-size:13px;font-weight:700;}}
.p-legs{{font-size:11px;color:var(--dim);font-weight:600;background:var(--glass);padding:3px 8px;border-radius:999px;border:1px solid var(--border);}}
.mix-tag{{font-size:9px;font-weight:700;background:rgba(52,211,153,0.15);color:var(--accent);padding:2px 6px;border-radius:999px;border:1px solid rgba(52,211,153,0.35);letter-spacing:.04em;vertical-align:middle;margin-left:4px;}}
.pd{{font-size:11px;color:var(--dim);margin-bottom:6px;}}
.pleg{{display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-top:1px solid var(--border);}}
...
.disclaimer{{margin:28px 16px 16px;padding:14px 16px;background:var(--glass);border:1px solid var(--border);border-radius:var(--rs);border-left:3px solid var(--accent);}}
.disclaimer p{{font-size:12px;color:var(--dim);line-height:1.7;margin:0;}}
.disclaimer strong{{color:var(--soft);}}
.disclaimer .dyod{{font-size:13px;font-weight:800;color:var(--accent);display:block;margin-top:8px;}}
#scroll-track{{position:fixed;right:6px;top:50%;transform:translateY(-50%);height:60vh;width:6px;background:rgba(255,255,255,0.06);border-radius:999px;z-index:999;}}
#scroll-thumb{{position:absolute;left:50%;transform:translateX(-50%);width:24px;height:48px;background:rgba(52,211,153,0.35);border:1px solid rgba(52,211,153,0.6);border-radius:999px;cursor:grab;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;touch-action:none;transition:background .15s;}}
...
<div class="how-wrap">
  <button class="how-toggle" onclick="this.classList.toggle('open');document.getElementById('how-body').classList.toggle('visible')">
    <span>📖 How This System Works</span><span class="how-arrow">▾</span>
  </button>
  <div id="how-body" class="how-body">
    <div><div class="how-title">🎯 What is The Safe K Report?</div>
    <p>Every starting pitcher gets graded on a <strong>12-point system</strong> - projected strikeouts, how deep they go, walk rate, opposing lineup K tendencies, swing-and-miss rates, and more. The more green boxes, the higher the tier.</p></div>
    <div><div class="how-title">🔒 Why "Safe"?</div>
    <p>The O line we recommend is always the <strong>floor</strong> - the most conservative bet. A pitcher projecting 6 Ks gets <strong>O 3.5</strong>, not O 5.5. Even if they underperform a little, you still win. We never chase lines.</p></div>
    <div><div class="how-title">➕ How to use these plays</div>
    <p>These are designed as <strong>extra legs on a parlay</strong>. Tack on a Safe K leg to boost the payout without blowing up the slip. Low risk, real value.</p></div>
    <div><div class="how-title">📈 Can I go higher than the recommended line?</div>
    <p>Always your call. The tier tells you how much cushion you have:</p>
    <div class="how-grid">
      <div class="how-row"><span class="how-tier" style="color:#3b82f6">💎 Diamond</span><span class="how-desc">All criteria green. Comfortable going up one line.</span></div>
    ...
    <div><div class="how-title">⚡ The simple rule</div>
    <div class="rule-grid">
      ...
      <div class="rule-item"><span class="rule-badge" style="background:rgba(59,130,246,0.15);border-color:rgba(59,130,246,0.4);color:#3b82f6">💎 or 🏆 tier</span><span class="rule-desc">Can consider going up one line</span></div>
      <div class="rule-item"><span class="rule-badge" style="background:rgba(239,68,68,0.12);border-color:rgba(239,68,68,0.4);color:#ef4444">Never skip more than +1</span><span class="rule-desc">Don't jump two lines</span></div>
    </div></div>
  </div>
</div>
<div class="snav">
...
<div class="disclaimer">
  ...
  A scratch or bullpen day makes these grades invalid instantly. Check your book before locking anything in.</p>
  <p style="margin-top:8px;">The O lines shown are <strong>floor lines</strong> - the safest option.
  Moving up is always <strong>optional</strong> and based on your own read.
...
print(f'  Phase 2: {"Active" if PHASE2_ACTIVE else "Pending"}')
print(f'  Diamonds ({len(diamonds)}): {[p["name"] for p in diamonds]}')
print(f'  Elites   ({len(elites)}): {[p["name"] for p in elites]}')
